...
def validate_dtype(dic,df):
    ...
    for i,j in dic.items():
        ...
        try:
            assert test_result.success, \
            f"{i} Column is {test_result.result['observed_value']} and must be {j}: xxFAILED"
...
def columns_without_null_(col,df):
    raw_test_df = SparkDFDataset(df)
    for i in col:
        test_result = raw_test_df.expect_column_values_to_not_be_null(i)
...
def columns_values_between_(col,df,min_val,max_val):
    raw_test_df = SparkDFDataset(df)
    for i in col:
        test_result = raw_test_df.expect_column_values_to_be_between(i,min_val,max_val)
        assert test_result.success,\
...
def columns_unique_values_(col,df):
